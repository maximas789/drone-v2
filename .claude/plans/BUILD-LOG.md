...
| 9 — Prove it | F31 | 🟨 **In progress (Session 46). F31a — the mechanical gate — is done and every one of the eleven checks passed**: typecheck, no schema drift, migrations onto a *fresh* database (7 → 25 tables, 16 enums), build (63 routes, nine prerendered and all session-free), lint, **1114 tests**, 2127 i18n keys, a production serve on 3001, **126/126 route assertions**, **25/25 ownership assertions in both directions**, and **12/12 with `RESEND_API_KEY` and `BLOB_READ_WRITE_TOKEN` deleted before any import** — including a real serial-less approval that minted `AJN-308N-1W5B` with a three-year window. Evidence in `docs/VERIFICATION.md`, re-runnable as `pnpm verify:{fresh-db,routes,two-accounts,no-keys}`. **F31b (the 14-step walkthrough by hand in Arabic, plus 375 px) and F31c (four critics, two rounds) remain**, and thread 64 — a reviewer signing in — is now the only thing between F31 and a complete pass. **F31b is started but blocked (Session 46b): step 9 is done and is 46/46** — all 5 Remote IDs across 3 anonymous surfaces each, every body searched whole for 12 forbidden strings, **zero identity leaks**, done as source reading because `display:none` is not redaction. Two false positives recorded: `0501234567` is the **message catalogue's own example number**, which the demo data reuses, and a `429` is `rid.resolve`'s limiter (30/min/IP), now asserted rather than mistaken for a broken page. Step 10's trail checked — 3 reveal events, all with actor, role, reason and IP hash; the 5-character `"owner"` reason is F28a's self-reveal marker, deliberate. **Steps 1–8, 11–14 and the 375 px pass are blocked on the Chrome extension, which is not connected.** Also found and not fixed: **all five demo aircraft are named `PROBE18B …`** — hidden from anonymous scanners, but it is what a reviewer sees. **Step 7 added: the QR proved without a decoder, 14/14** — each stored PNG byte-identical to a fresh render of `{APP_URL}/ar/rid/{code}`, 512×512, with a **negative control** so the comparison means something. Only 3 of 5 codes have a QR and it is a seeding artefact, not a defect; a missing one degrades to a Regenerate panel. Every sticker points at localhost — fatal in production. |
