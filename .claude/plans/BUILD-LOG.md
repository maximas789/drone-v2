...
- **Open finding: the Blob store is in `iad1`, the functions and database in `sin1`.** Each `/api/files/…` read does a `head` and a `fetch` against Washington. Unmeasured under `sin1`. A store's region cannot be changed; the fix, if the numbers justify it, is a new store nearer Singapore and a copy of the files.
