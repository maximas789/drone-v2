...
2. **Arabic-Indic digits in the planned-altitude input.** On `/ar/bookings/new` step 4 the field renders **١٢٠** while the ceiling hint immediately below renders **120**. The stored value is ASCII — codepoints `31 32 30` — so `innerText` and every test see "120" and **only a screenshot catches it**, the same family as the `<bdi>` and `text-anchor` traps. Eliminated as causes: text direction (flipping to rtl changed nothing), `lang`, and the `locl` font feature (`"locl" 0` changed nothing). Both elements report the same computed family and feature settings. **The mechanism is unresolved** and the leading suspect is the resolved face for form controls versus prose — numeric inputs elsewhere (`step-specs.tsx`'s weight) carry `font-mono` and render Latin, while this one does not. Not fixed because a guessed fix here is worse than a named finding.
