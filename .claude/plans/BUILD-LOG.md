...
- **QR storage is Vercel Blob.** Store `drone-v2-blob`, Active, 6 blobs / 628 KB. The three QR PNGs (`qr/AJN-QQP3-SMD1`, `qr/AJN-4R7J-6A8G`, `qr/AJN-S2AT-298S`) each answer 200 `image/png` from the store's public base URL; an unknown code answers 404. (First time the production Blob driver has been shown to have run.)
- **`verify:qr`, production equivalent — 3/3 + negative control.** `scripts/verify/qr.mts` cannot be pointed at production: it reads local `uploads/qr/*.png` and never touches the database or Blob. So the three real QR PNGs were downloaded from the store's public URL and byte-compared with a fresh `renderQrPng` under `APP_URL=https://drone-v2.vercel.app`: all three identical; a wrong code does not match. Every production sticker therefore encodes `https://drone-v2.vercel.app/ar/rid/<code>` — the trap CLAUDE.md names is closed for these. (The compare was a one-off script kept out of the repo; if this becomes a gate, `qr.mts` needs a `--blob` mode.)
- **Signed-in pages re-timed under `sin1`** (browser signed in as the admin/reviewer account, from Riyadh): drones 0.30–0.47 s, dashboard 0.36–0.60 s, notifications ~0.36 s, bookings ~0.32 s, settings ~0.32 s, `get-session` 0.24–0.47 s — every response served from `sin1`, against 3–4 s before the region fix. `/api/files` for a QR PNG (owner-or-reviewer check, then Blob `head` + `fetch`): 0.95 s cold, 0.49 s warm; the Blob CDN directly: 0.50 s. Not measured: the QR retry action, and larger drone photos.
- **`verify:qr --blob` built (the repo gate), 2026-09-20.** `scripts/verify/qr.mts` now takes `--blob <public-store-url> <code>...`: it downloads each `qr/{code}.png` from the public store (no secret) and runs the same checks as the disk mode, with `APP_URL` given inline. In that mode a `localhost` payload is a **failure**, not a note. **Verified both ways:** local mode unchanged, 22/22; the three real production stickers 18/18 (byte-identical to a fresh render at `https://drone-v2.vercel.app`); and it **fails** when it should — an unknown code (HTTP 404), `APP_URL` not given (the stickers "point somewhere else"), no codes. **It also found a flaw in the old check:** "is exactly 512 px" failed the real stickers at 511×511 while they were byte-identical to a fresh render, because `qrcode` rounds the width to whole modules and the production URL is longer than `localhost`. It only ever passed locally by luck of the hostname; the assertion is now "square, within 2 px of 512". The codes must be passed as arguments (a public store can't be listed without its token). `typecheck` and eslint on the script pass. Documented in `CLAUDE.md`.
- **Open finding (now low priority): the Blob store is in `iad1`, the functions and database in `sin1`.** Each `/api/files/…` read does a `head` and a `fetch` against Washington. Unmeasured under `sin1`. A store's region cannot be changed; the fix, if the numbers justify it, is a new store nearer Singapore and a copy of the files.
- **Pitch pass (honesty rules), 2026-09-20, commit `e22a5e9`.** Checked: the proposal / not-endorsed notice in every place it should appear (footer, auth pages, scan page, email footer, print sticker, social preview, `llms.txt`, docs, legal) in both languages; the JSON-LD (deliberately bare); no invented numbers, testimonials or logos (the landing demo card uses a reserved, never-issuable code); the "not official airspace" notice on every map surface; "human review, no SMS, no automatic verification" wording. **Four fixes:** the on-screen Remote ID card had no notice next to "Registration valid" (added under the card); landing step "Fly within the booking" said "an authorised flight" (now "a booked flight window"); the Arabic identity-check line said "موظف مختص" (a specialist employee, implying official staff — now "أحد المراجعين"); "the regulatory record" became "this system's record". `i18n:check` 2135 keys and 1114 tests pass; `typecheck` passes. **Not done:** a read of all ~2100 Arabic keys (needs an Arabic reader); the card page not looked at live after deploy. **`pnpm lint` fails for a reason outside this work:** 82 errors, all inside an untracked stray copy of the repo at `.kilo/worktrees/level-porcupine/`; eslint on the changed file is clean.
- Two test drones (`QR Test`, `RESEND_TEST`) remain in production under the pilot account. **Kept as demo data on the owner's decision.** They cannot be deleted through the app — `deleteDroneAction` allows drafts only, and a submitted registration is part of the append-only audit trail — and a raw row delete would orphan their audit events and their Blob files. Admin revoke is the option if the list is ever wanted cleaner.
