...
**Date:** 2026-08-25
**Status:** 🟨 **Round one complete and fixed; round two blocked on the account's monthly spend limit.** 13 broken, 5 missing, 25 worth knowing.
...
#### Round two — blocked, and said so

**Both round-two critics terminated on their first call: the account hit its monthly spend limit.** Round two therefore became the author checking their own fixes, which `docs/VERIFICATION.md` records as the weaker evidence it is. All twelve fixes were verified against the source with the grep that shows each, and the two correctness questions round two existed to ask were answered: `clearRefusal` cannot race `applyReasons`, and the two `isDev` derivations are byte-identical.
...
- **Still open**: `booking-rejected` has no caller; the job log has no filter or pagination; the 375 px header and the 120 m claim are the user's decisions.
