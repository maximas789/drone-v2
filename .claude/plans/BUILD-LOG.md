...
| 7 — Admin | F22–F25 | 🟡 **In progress (Sessions 26–30).** **F22 complete.** **F23 split a/b/c; F23a and F23b done** — the geometry layer and the zone list, then the Sunday-first hours grid, the live slot preview through the real `deriveSlots`, and the publish/suspend/archive lifecycle with its impact warnings. F23c (closures, cities) remains, then F24 and F25. |
...
| 68 | **`zone-suspended.ts` has never run.** The suspension itself is driven and proven over HTTP, but `inngest-cli` is not installed on this machine, so the fan-out that cancels each future booking and emails each pilot is written, registered in `functions/index.ts`, and unexecuted. It is a near-copy of `closure-fanout.ts`, which has run — but "near-copy" is not "ran". The same gap will apply to F23c's closure preview until somebody starts an Inngest dev server. | F23b | F23c, F31 |
...
- **Any Inngest run at all on this machine** — `inngest-cli` is not installed, so `zone-suspended.ts` (F23b) and F23c's closure fan-out cannot be executed locally. `inngest.send` itself fails, which is thread 69's finding.
